...
class CheckRegistrationCodeTest extends TestCase
{
    ...
    public function testExecuteUsernameInvalid()
    {
        ...
        $code = base64_encode(sprintf('email-%d-%s', $validity->getTimestamp(), md5('ndewez.secret')));
    ...
    {
        $validity = (new \DateTime())->add(new \DateInterval(sprintf('PT%dH', RegistrationCode::VALIDITY)));
        $code = base64_encode(sprintf('email-%d-%s', $validity->getTimestamp(), md5('ndewez.secret2')));
    ...
    public function testExecuteRegistrationNotInProgress()
    {
        ...
        $validity = (new \DateTime())->add(new \DateInterval(sprintf('PT%dH', RegistrationCode::VALIDITY)));
        $code = base64_encode(sprintf('email-%d-%s', $validity->getTimestamp(), md5('email.secret')));
